...
export function expensesInCurrentYear(expenses: Expense[]) {
  ...
  return expenses.filter((expense) => new Date(expense.date).getFullYear() === currentYear).reduce((acc, expense) => acc + expense.expense, 0);
}
...
export function expensesInCurrentMonth(expenses: Expense[]) {
  ...
  return expenses.filter((expense) => new Date(expense.date).getMonth() === currentMonth).reduce((acc, expense) => acc + expense.expense, 0);
}
...
export function expensesInCurrentWeek(expenses: Expense[]) {
  ...
  return expenses.filter((expense) => new Date(expense.date).getDate() >= currentWeek).reduce((acc, expense) => acc + expense.expense, 0);
}
...
export function expensesToday(expenses: Expense[]) {
  ...
  return expenses.filter((expense) => expense.date === today).reduce((acc, expense) => acc + expense.expense, 0);
}
 ...
 * @returns string
 */
export function mostSpentCategory(expenses: {
  id: string;
  date: string;
  expense: number;
  categoryId: string;
  ...
  const categoryExpenses = uniqueCategories.map((category) => {
    return {
      category,
      expense: expenses.filter((expense) => expense.categoryName === category).reduce((acc, expense) => acc + expense.expense, 0)
    }
  })
  const sortedCategoryExpenses = categoryExpenses.toSorted((a, b) => b.expense - a.expense)
...
export function mostSpentDay(expenses: Expense[]) {
  ...
  const dayExpenses = uniqueDays.map((day) => {
    return {
      day,
      expense: expenses.filter((expense) => new Date(expense.date).getDay() === day).reduce((acc, expense) => acc + expense.expense, 0)
    }
  })
  const sortedDayExpenses = dayExpenses.toSorted((a, b) => b.expense - a.expense)
...
export function leastSpentDay(expenses: Expense[]) {
  ...
  const sortedDayExpenses = dayExpenses.toSorted((a, b) => a.expense - b.expense)
  return getDayName(sortedDayExpenses[0].day);
}